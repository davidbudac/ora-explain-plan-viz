...
description: >
  Video-first companion to Cobalt Grid's design.md, DARK-INVERTED for a dark developer tool.
  The unit is the frame (1920×1080). Atoms — deep-navy ground, near-white ink, electric-blue
  voltage, the permanent blue graph-paper grid, Inter 400 + ui-monospace, top/bottom hairlines,
  and the pixel-glitch + QR-block signatures. Data-semantic accents (red/amber/emerald) are used
  only for numbers that echo the app UI. Composition, frame scale, and aspect behavior rewritten
  for the frame. Motion is out of scope.
unit: the frame — 1920×1080 primary; 9:16 and 1:1 documented
principle: atoms are sacred · composition is free · numbers come from the script

colors:
  paper: "#0b1220"        # deep navy ground — matches the app's dark slate canvas
  paper-2: "#0f172a"      # secondary surface (slate-900) for inset cards / panels
  ink: "#e6edf7"          # near-white slate — primary type, rules, glitch, QR on dark
  ink-soft: "#5b9dff"     # electric blue — editorial subtitles + the scarce voltage
  grid: "rgba(80, 140, 240, 0.12)"       # faint blue blueprint graph-paper grid
  ink-faint: "rgba(230, 237, 247, 0.16)" # faint row dividers
  # --- data-semantic accents (used ONLY for numbers/callouts that mirror the app UI) ---
  hot: "#ef4444"          # red — the "problem" + hotspot beats
  warn: "#f59e0b"         # amber — cardinality mismatch
  good: "#34d399"         # emerald — improvements / positive deltas

typography:
  # — reading ramp —
  body:        { fontFamily: "Inter", cqw: 0.83, weight: 400, lineHeight: 1.5 }
  body-lede:   { fontFamily: "Inter", cqw: 0.95, weight: 400, lineHeight: 1.5 }
  micro:       { fontFamily: "Inter", px: 13, weight: 600, tracking: "0.16em", upper: true }
  micro-strong:{ fontFamily: "Inter", px: 16, weight: 600, tracking: "0.18em", upper: true }
  mono-tag:    { fontFamily: "ui-monospace", cqw: 0.78, weight: 400, tracking: "0.05em" }
  mono-chrome: { fontFamily: "ui-monospace", px: 13, weight: 400, tracking: "0.06em" }
  # — display / hero ramp (Inter 400, negative tracking) —
  table-name:  { fontFamily: "Inter", cqw: 1.5, weight: 400, lineHeight: 1.15 }
  row-headline:{ fontFamily: "Inter", cqw: 2.1, weight: 400, lineHeight: 1.05 }
  ed-callout:  { fontFamily: "Inter", cqw: 2.6, weight: 400, lineHeight: 1.1, italic: true }
  headline:    { fontFamily: "Inter", cqw: 4.6, weight: 400, lineHeight: 0.95 }
  headline-index:{ fontFamily: "Inter", cqw: 5.0, weight: 400, lineHeight: 0.95 }
  display-quote:{ fontFamily: "Inter", cqw: 5.7, weight: 400, lineHeight: 1.05, tracking: "-0.005em" }
  display-chapter:{ fontFamily: "Inter", cqw: 6.8, weight: 400, lineHeight: 1.0, tracking: "-0.005em" }
  display-closing:{ fontFamily: "Inter", cqw: 9.4, weight: 400, lineHeight: 0.96, tracking: "-0.005em" }
  display-hero:{ fontFamily: "Inter", cqw: 10.4, weight: 400, lineHeight: 0.9, tracking: "-0.008em" }
  vbig-numeral:{ fontFamily: "Inter", cqw: 12.5, weight: 400, lineHeight: 0.9, tracking: "-0.015em" }
...
components:
  ...
  ledger-row:
    layout: "grid: mono num · Inter name · Hanken desc · mono delta"
...
- **Fonts** — already set to **Inter** (display) / **ui-monospace** (body); ignore any preset font name lingering in prose.
- **Colors** — use the frontmatter hex; preset color NAMES in prose (e.g. "cobalt", "cream") mean the remapped brand values.
- **DARK INVERSION (important).** This preset was authored light (cream paper / cobalt ink). It has been **hand-inverted to a dark blueprint** to sit flush with the app's dark UI screenshots: `paper` is a deep navy ground, `ink` is near-white slate, `ink-soft` is electric blue (the scarce voltage), and `grid` is a faint blue graph-paper grid. Read every "cream/cobalt/paper" mention in the prose below as "dark-navy ground / near-white ink / blue voltage."
- **Accent-hue exception.** The source is strictly two-color. This adaptation adds three **data-semantic** accents — `hot` (red), `warn` (amber), `good` (emerald) — for numbers/badges that echo the real app UI ONLY (a hotspot ring, a cardinality "634.9x" callout, a positive delta). They are scarce voltage, never decoration. Base frames stay navy + near-white + one blue.
...
The voice is a three-face conversation: **Inter** serif at weight 400 (size, not weight,
makes hierarchy) carries every display and headline; **Inter** carries body and
uppercase tracked labels; **ui-monospace** carries all chrome — page numbers, tags, ticks, vertical
...
- **Inter 400** for all display (negative tracking); **Hanken 600** uppercase 0.16em labels; **ui-monospace** chrome.
...
- **Squint** — one Inter element dominates at **3–6× its nearest neighbor** (`display-hero`/`vbig-numeral`); hierarchy is size, never weight.
...
Hanken→Inter, from a mono delta arrow, or from opacity, never from color.

## Typography

Two ramps. The **reading ramp** (Hanken body 0.83cqw, mono chrome in px) carries copy + labels;
the **display/hero ramp** (Inter `headline` 4.6cqw → `vbig-numeral` 12.5cqw) carries every
...
- **Inter at 400 only**, in cobalt, with negative tracking (hero −0.008em, numeral −0.015em). **Hanken labels uppercase, 0.16–0.18em.** **ui-monospace 0.04–0.08em.** No bold serif.
...
**Ground** paper + grid, hairlines. **Composes** pixel-glitch (right, ~26cqw), qr-block (top-right), display-hero, ed-callout. **Focal** a 1–2 line `display-hero` Inter headline in cobalt, left-anchored, with an italic `ed-callout` subtitle in `{colors.ink-soft}`. **Chrome** Hanken kicker; mono meta top + page number. **Accent** none (cobalt is the only ink). **Silence** ~45% paper. **Fixed** Inter 400, glitch + QR present, grid + hairlines. **Free** title, glitch step pattern, QR placement. **Density** sparse.

### 2 · Index Ledger (catalog · move: dense matrix · left — the dense frame)

**Ground** paper + grid, hairlines, `pad-top`/`pad-bottom`. **Composes** topbar-rule, ledger-rows. **Focal** a topbar (`headline-index` + mono lab-tag, 1.5px rule) over 4–6 ledger rows (mono num · Inter name · Hanken desc), ink-faint dividers. **Chrome** page number. **Accent** mono delta arrows. **Silence** tight — the density exception (the grid wants filling). **Fixed** 1.5px topbar rule, 1px ink-faint dividers, Inter names. **Free** rows, lab-tag, deltas. **Density** dense-exception.

### 3 · Chapter Opener (section · move: scale · sparse · left)

**Ground** paper + grid, hairlines. **Composes** pixel-glitch (left, ~14cqw, low opacity), mono index, display-chapter, body-lede. **Focal** a `display-chapter` Inter title, with a small mono `CHAPTER NN` index above and an optional Hanken lede ≤42cqw. **Accent** none. **Silence** ~60% — let the grid breathe. **Fixed** Inter 400, glitch low-opacity, grid showing. **Free** title, lede, glitch side. **Density** sparse.
...
**Ground** paper + grid, hairlines. **Composes** display-quote (or display-manifesto), attribution rule, optional compact glitch. **Focal** a 2–3 line Inter pull in cobalt, with a Hanken kicker above and a 1px cobalt attribution rule + mono byline beneath. **Accent** none. **Silence** ~55% — deliberately open. **Fixed** Inter 400, attribution rule, grid showing. **Free** quote, byline. **Density** sparse.

### 6 · Colophon (closer · move: right-aligned close · sparse)

**Ground** paper + grid, hairlines. **Composes** pixel-glitch (left edge, mirroring the cover), display-closing, mono credit columns. **Focal** a right-aligned `display-closing` Inter title with a Hanken kicker above. **Chrome** 3–4 column mono credit grid at the foot; page number. **Accent** none. **Silence** ~50%. **Fixed** glitch on left, Inter 400. **Free** closing line, credits. **Density** sparse.
...
- Set **Inter at 400 in cobalt**; make hierarchy with size, not weight; negative-track display.
- Track **Hanken labels uppercase 0.16em**; **ui-monospace chrome 0.04–0.08em**.
...
- Don't bold Inter, round any corner, or add a drop shadow (QR outset is an anti-shadow only).
...
- **Squint** — one Inter element dominates at 3–5× its neighbor.
...
- **Type** — Inter 400 negative-tracked, fit-to-measure; Hanken labels 0.16em; ≥1.4cqw floor.
...
- **Three Google Fonts** (Inter, Inter, ui-monospace); CJK pairing (Noto Serif SC 700/400) carries over from the source.
...
## Fonts — INLINE THIS BLOCK VERBATIM (inside your `<template>`'s `<style>`)

Two families ship as real files under `assets/fonts/` (paths resolve from the project root at render).
The `ui-monospace` role in the type ramp = **"JetBrains Mono"**. Every frame MUST inline this exact
`@font-face` block so the headless render never falls back. Do not name any other font.

```css
@font-face{font-family:'Inter';font-style:normal;font-weight:400;src:url('assets/fonts/Inter-400.woff2') format('woff2');font-display:block}
@font-face{font-family:'Inter';font-style:normal;font-weight:500;src:url('assets/fonts/Inter-500.woff2') format('woff2');font-display:block}
@font-face{font-family:'Inter';font-style:normal;font-weight:600;src:url('assets/fonts/Inter-600.woff2') format('woff2');font-display:block}
@font-face{font-family:'Inter';font-style:normal;font-weight:700;src:url('assets/fonts/Inter-700.woff2') format('woff2');font-display:block}
@font-face{font-family:'Inter';font-style:normal;font-weight:800;src:url('assets/fonts/Inter-800.woff2') format('woff2');font-display:block}
@font-face{font-family:'Inter';font-style:normal;font-weight:900;src:url('assets/fonts/Inter-900.woff2') format('woff2');font-display:block}
@font-face{font-family:'Inter';font-style:italic;font-weight:500;src:url('assets/fonts/Inter-500-italic.woff2') format('woff2');font-display:block}
@font-face{font-family:'Inter';font-style:italic;font-weight:600;src:url('assets/fonts/Inter-600-italic.woff2') format('woff2');font-display:block}
@font-face{font-family:'JetBrains Mono';font-style:normal;font-weight:400;src:url('assets/fonts/JetBrainsMono-400.woff2') format('woff2');font-display:block}
@font-face{font-family:'JetBrains Mono';font-style:normal;font-weight:500;src:url('assets/fonts/JetBrainsMono-500.woff2') format('woff2');font-display:block}
@font-face{font-family:'JetBrains Mono';font-style:normal;font-weight:700;src:url('assets/fonts/JetBrainsMono-700.woff2') format('woff2');font-display:block}
```
