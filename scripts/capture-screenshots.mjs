...
async function main() {
  ...
  // ---- 4. cardinality.png — worst E-Rows vs A-Rows mismatch node selected
  console.log('shot 4: cardinality.png');
  {
    // Open the "Cardinality Mismatch" hotspots accordion (native <details>).
    const found = await evaluate(`(() => {
      const d = [...document.querySelectorAll('details')].find(d => d.querySelector('summary h4')?.textContent.trim() === 'Cardinality Mismatch');
      if (d) d.open = true;
      return !!d;
    })()`);
    if (!found) throw new Error('cardinality.png: hotspots Cardinality Mismatch accordion not found');
    await sleep(200);
    // Click the first (worst) entry in that accordion.
    await clickEl(
      `[...document.querySelectorAll('details')].find(d => d.querySelector('summary h4')?.textContent.trim() === 'Cardinality Mismatch')?.querySelector('div button')`,
      { label: 'worst cardinality mismatch entry' }
    );
    await sleep(600);
    // Context-driven selection doesn't add React Flow's .selected class, so
    // verify via the detail panel: the severity badge span must be present.
    await verify(
      `[...document.querySelectorAll('span')].some(el => el.textContent.trim() === 'Cardinality Mismatch')`,
      'cardinality mismatch badge in detail panel'
    );
  }
  ...
  // Keep the default E-Rows flow metric: example 05's A-Time concentrates in
  // one operation and renders as featureless slabs.
  await sleep(1500);
  ...
  await sleep(300);
  await clickEl(
    `[...document.querySelectorAll('button')].find(b => b.textContent.trim() === 'SQL Monitor XML (Nested Loops)')`,
    { label: 'example 09 menu entry' }
  );
  await waitFor(
