      ...
      {plans.map((slot, index) => {
        ...
        return (
          <div
            key={slot.id}
            className={`
              flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-sm transition-colors border cursor-pointer
              ${isActive
          ...
          >
            ...
            >
              ...
              {phv && (
                <span className={`font-mono text-[10px] ${isActive ? 'text-blue-200' : 'text-[var(--text-muted)] dark:text-[var(--text-muted-dark)]'}`}>
                  PHV: {phv}
                </span>
              )}
              {!slot.parsedPlan && (
                <span className={`text-[10px] italic ${isActive ? 'text-blue-200' : 'text-[var(--text-muted)] dark:text-[var(--text-muted-dark)]'}`}>
                  (empty)
                </span>
              )}
            </button>
            <button
              ...
              className={`
                ...
                ${isActive
                  ...
                  : 'hover:bg-[var(--surface-raised)] dark:hover:bg-[var(--surface-dark)] text-[var(--text-muted)] dark:text-[var(--text-muted-dark)]'
                }
              `}
