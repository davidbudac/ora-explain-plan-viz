        ...
        return (
          ...
          >
            ...
            >
              ...
              {phv && (
                <span className={`font-mono text-[10px] ${isActive ? 'text-cyan-200' : 'text-[var(--text-muted)] dark:text-[var(--text-muted-dark)]'}`}>
                  PHV: {phv}
                </span>
              )}
              {!slot.parsedPlan && (
                <span className={`text-[10px] italic ${isActive ? 'text-cyan-200' : 'text-[var(--text-muted)] dark:text-[var(--text-muted-dark)]'}`}>
                  (empty)
                </span>
              )}
            </button>
            <button
              ...
              className={`
                ml-1 p-0.5 rounded-lg transition-colors
                ${isActive
                  ? 'hover:bg-cyan-500 text-cyan-200'
                  : 'hover:bg-[var(--surface-raised)] dark:hover:bg-slate-700 text-[var(--text-muted)] dark:text-[var(--text-muted-dark)]'
                }
              `}
