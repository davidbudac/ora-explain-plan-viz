  ...
  if (selectedNodes.length === 0) {
    return (
      ...
      >
        ...
        />
        <div className="p-3 border-b border-[var(--border-color)] dark:border-[var(--border-color-dark)] bg-[var(--surface-raised)] dark:bg-[var(--surface-raised-dark)]/40">
            ...
            <div className="space-y-1">
              {worstNodes.byTime.map(n => (
                <button
                  ...
                  className="w-full text-left px-2 py-1.5 text-xs rounded hover:bg-[var(--surface-raised)] dark:hover:bg-[var(--surface-raised-dark)] transition-colors flex items-center justify-between gap-2"
                >
                  <span className="flex items-center gap-1.5 truncate">
                    <span className="w-5 h-5 rounded-full bg-indigo-500 text-white dark:bg-indigo-500 dark:text-white text-[10px] font-bold flex items-center justify-center shrink-0">{n.id}</span>
            ...
            <div className="space-y-1">
              {annotations.groups.map((group) => {
                ...
                return (
                  <div
                    key={group.id}
                    className="flex items-center justify-between px-2 py-1.5 text-xs rounded hover:bg-[var(--surface-raised)] dark:hover:bg-[var(--surface-raised-dark)] transition-colors"
                  >
                    <span className="flex items-center gap-1.5 truncate">
                    ...
                    <button
                      ...
                      className="p-0.5 hover:bg-[var(--border-color)] dark:hover:bg-[var(--border-color-dark)] rounded"
                      title="Edit group"
                    >
      ...
      >
        ...
        <div className="p-3 border-b border-[var(--border-color)] dark:border-[var(--border-color-dark)] bg-[var(--surface-raised)] dark:bg-[var(--surface-raised-dark)]/40">
          <div className="flex items-start justify-between">
            <div>
              <span className="inline-block px-2 py-0.5 rounded text-[11px] font-semibold border border-[var(--border-color)] dark:border-[var(--border-color-dark)] text-[var(--text-secondary)] dark:text-[var(--text-secondary-dark)] bg-[var(--surface)] dark:bg-[var(--surface-dark)]">
                Multi Selection
              </span>
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={() => setIsCollapsed(true)}
                className="p-1 hover:bg-[var(--surface-raised)] dark:hover:bg-[var(--surface-raised-dark)] rounded"
                title="Collapse panel"
              >
                <svg className="w-4 h-4 text-[var(--text-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 5l7 7-7 7M5 5l7 7-7 7" />
        ...
        <div className="p-3 border-b border-[var(--border-color)] dark:border-[var(--border-color-dark)]">
          ...
          <button
            ...
            className="mt-2 w-full px-2.5 py-1.5 text-xs font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 border border-blue-200 dark:border-blue-800 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors"
          >
        ...
        <div className="p-3 border-b border-[var(--border-color)] dark:border-[var(--border-color-dark)]">
          ...
          <div className="h-2.5 bg-[var(--surface-raised)] dark:bg-[var(--surface-raised-dark)] rounded-full overflow-hidden">
            <div
      ...
      <div className={`p-3 border-b border-[var(--border-color)] dark:border-[var(--border-color-dark)] ${colors.bg}`}>
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="mt-2 flex items-center gap-2">
              <span className="w-6 h-6 rounded-full bg-indigo-500 text-white dark:bg-indigo-500 dark:text-white text-xs font-bold flex items-center justify-center">
                {node.id}
              </span>
        ...
        {(node.queryBlock || node.objectAlias) && (
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            {node.objectAlias && (
              <span className="px-2 py-0.5 rounded text-xs font-medium bg-[var(--surface-raised)] dark:bg-[var(--surface-raised-dark)] text-[var(--text-secondary)] dark:text-[var(--text-secondary-dark)]">
                {node.objectAlias}
              </span>
            )}
          </div>
        ...
        <div className="p-3">
          ...
          {node.accessPredicates && (
            <div className="mb-3">
              ...
              <code className="block text-xs bg-[var(--surface-raised)] dark:bg-[var(--surface-raised-dark)] p-2 rounded border border-[var(--border-color)] dark:border-[var(--border-color-dark)] text-[var(--text-primary)] dark:text-[var(--text-primary-dark)] font-mono whitespace-pre-wrap break-all">
...
function StatItem({ label, value, highlight }: { label: string; value?: string; highlight?: 'blue' | 'purple' | 'orange' }) {
  ...
  const highlightStyles = {
    blue: 'bg-indigo-50 dark:bg-indigo-900/20 border border-blue-200 dark:border-blue-800',
  ...
  return (
    <div className={`rounded-lg p-2 border border-[var(--border-color)] dark:border-[var(--border-color-dark)] ${highlight ? highlightStyles[highlight] : 'bg-[var(--surface-raised)] dark:bg-[var(--surface-raised-dark)]'}`}>
...
function CopyButton({ text, label }: { text: string; label?: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="p-1 hover:bg-[var(--border-color)] dark:hover:bg-[var(--border-color-dark)] rounded transition-colors"
