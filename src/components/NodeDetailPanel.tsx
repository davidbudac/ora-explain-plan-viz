...
export function NodeDetailPanel({ panelWidth, onResizeStart }: NodeDetailPanelProps) {
  ...
  if (isCollapsed) {
    return (
      <div className="bg-white dark:bg-neutral-900 border-l border-neutral-200 dark:border-neutral-800 flex flex-col items-center py-3">
        <button
        ...
        <span className="text-[11px] text-neutral-500 dark:text-neutral-400 mt-2 writing-mode-vertical">Details</span>
      </div>
    );
  }
  ...
  if (isMultiSelection && aggregateSelection) {
    ...
    return (
      ...
      >
        ...
        <div className="p-3 border-b border-neutral-200 dark:border-neutral-800">
          <h4 className="text-xs font-semibold text-neutral-700 dark:text-neutral-300 mb-2 tracking-wide">Annotate</h4>
          <BulkHighlightPicker
        ...
        <div className="p-3 border-b border-neutral-200 dark:border-neutral-800">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-semibold text-neutral-700 dark:text-neutral-300 tracking-wide">{indicator.title}</span>
            <span className="text-xs text-neutral-600 dark:text-neutral-400">{indicator.percentText}% {indicator.referenceLabel}</span>
          </div>
          <div className="mb-2 text-xs font-medium text-neutral-700 dark:text-neutral-300">{indicator.formattedValue}</div>
        ...
        {parsedPlan?.hasActualStats && (
          <div className="p-3 border-b border-neutral-200 dark:border-neutral-800">
            <h4 className="text-xs font-semibold text-neutral-700 dark:text-neutral-300 mb-2 tracking-wide">Actual Statistics (Selection)</h4>
        ...
        {(aggregateSelection.sumMemoryUsed > 0 ||
          ...
          <div className="p-3 border-t border-neutral-200 dark:border-neutral-800">
            <h4 className="text-xs font-semibold text-neutral-700 dark:text-neutral-300 mb-2 tracking-wide">Memory & I/O (Selection)</h4>
            <div className="grid grid-cols-2 gap-3">
  ...
  return (
    ...
    >
      ...
      <div className={`p-3 border-b border-neutral-200 dark:border-neutral-800 ${colors.bg}`}>
        ...
        {(node.queryBlock || node.objectAlias) && (
          <div className="mt-2 flex flex-wrap gap-2">
            ...
            {node.objectAlias && (
              <span className="px-2 py-0.5 rounded text-xs font-medium bg-neutral-200 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-200">
                {node.objectAlias}
              </span>
            )}
          </div>
        ...
        {(() => {
          ...
          return tip ? (
            <div className="mt-2 text-[11px] text-neutral-500 dark:text-neutral-400 italic leading-snug">
              {tip}
            </div>
          ) : null;
        })()}
      ...
      {parsedPlan?.hasActualStats && (
        <div className="p-3 border-b border-neutral-200 dark:border-neutral-800">
          <h4 className="text-xs font-semibold text-neutral-700 dark:text-neutral-300 mb-2 tracking-wide">Actual Statistics</h4>
          <div className="grid grid-cols-2 gap-3">
      ...
      {(node.accessPredicates || node.filterPredicates) && (
        <div className="p-3">
          <h4 className="text-xs font-semibold text-neutral-700 dark:text-neutral-300 mb-2 tracking-wide">Predicates</h4>
...
function StatItem({ label, value, highlight }: { label: string; value?: string; highlight?: 'blue' | 'purple' | 'orange' }) {
  ...
  return (
    <div className={`rounded-md p-2 border border-neutral-200 dark:border-neutral-700 ${highlight ? highlightStyles[highlight] : 'bg-neutral-50 dark:bg-neutral-950'}`}>
      <div className="text-[11px] text-neutral-500 dark:text-neutral-400 tracking-wide">{label}</div>
      <div className={`text-xs font-semibold ${highlight ? valueStyles[highlight] : 'text-neutral-900 dark:text-neutral-100'}`}>{value}</div>
    </div>
  );
}
