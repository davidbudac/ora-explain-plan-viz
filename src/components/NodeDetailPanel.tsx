...
export function NodeDetailPanel({ panelWidth, onResizeStart }: NodeDetailPanelProps) {
  ...
  if (isCollapsed) {
    return (
      <div className="bg-[var(--surface)] border-l border-[var(--border-color)] flex flex-col items-center py-3">
        <button
          onClick={() => setIsCollapsed(false)}
          className="h-8 w-8 flex items-center justify-center hover:bg-[var(--border-highlight)] rounded-md transition-colors border border-[var(--border-color)]"
          title="Show details"
        >
        ...
        <span className="text-[11px] text-[var(--text-muted)] mt-2 writing-mode-vertical">Details</span>
      </div>
    );
  }
  ...
  if (selectedNodes.length === 0) {
    return (
      <div
        className="relative shrink-0 bg-[var(--surface)] border-l border-[var(--border-color)] overflow-y-auto"
        style={{ width: panelWidth }}
      >
        <button
          type="button"
          onPointerDown={onResizeStart}
          className="absolute left-0 top-0 z-10 h-full w-2 cursor-col-resize touch-none bg-transparent hover:bg-[var(--border-highlight)] transition-colors"
          aria-label="Resize details panel"
          title="Resize details panel"
        />
        <div className="p-3 border-b border-[var(--border-color)] bg-[var(--surface-raised)]">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold text-sm text-[var(--text-primary)]">Hotspots</h3>
            <button
              onClick={() => setIsCollapsed(true)}
              className="p-1 hover:bg-[var(--border-highlight)] rounded transition-colors"
          ...
          <p className="text-[11px] text-[var(--text-muted)] mt-1">Click a row to select, or Cmd/Ctrl-click nodes for multi-select</p>
        </div>

        {/* Worst by A-Time */}
        {worstNodes.byTime.length > 0 && (
          <div className="p-3 border-b border-[var(--border-color)]">
            <h4 className="text-xs font-semibold text-[var(--text-secondary)] mb-2 tracking-wide flex items-center gap-1">
            ...
            <div className="space-y-1">
              {worstNodes.byTime.map(n => (
                <button
                  ...
                  className="w-full text-left px-2 py-1.5 text-xs rounded hover:bg-[var(--surface-raised)] transition-colors flex items-center justify-between gap-2"
                >
                  <span className="flex items-center gap-1.5 truncate">
                    <span className="w-5 h-5 rounded-full bg-emerald-600 text-white text-[10px] font-bold flex items-center justify-center shrink-0">{n.id}</span>
                    <span className="truncate text-[var(--text-secondary)]">{n.operation}</span>
                  </span>
                  <span className="text-purple-400 font-medium whitespace-nowrap">{formatTimeCompact(n.actualTime)}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {worstNodes.byCost.length > 0 && (
          <div className="p-3 border-b border-[var(--border-color)]">
            <h4 className="text-xs font-semibold text-[var(--text-secondary)] mb-2 tracking-wide">Highest Cost</h4>
            <div className="space-y-1">
              {worstNodes.byCost.map(n => (
                ...
                >
                  ...
                  <span className="text-[var(--text-secondary)] font-medium whitespace-nowrap">{n.cost}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {worstNodes.byCardinalityMismatch.length > 0 && (
          <div className="p-3 border-b border-[var(--border-color)]">
            <h4 className="text-xs font-semibold text-[var(--text-secondary)] mb-2 tracking-wide">Worst Cardinality Mismatches</h4>
            <div className="space-y-1">
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`font-medium whitespace-nowrap ${
                      severity === 'bad' ? 'text-red-400' : 'text-amber-400'
                    }`}>{label}</span>
                  </button>
                );
              })}
        ...
        {annotations.groups.length > 0 && (
          <div className="p-3">
            <h4 className="text-xs font-semibold text-[var(--text-secondary)] mb-2 tracking-wide">Annotation Groups</h4>
            <div className="space-y-1">
              {annotations.groups.map((group) => {
                ...
                return (
                  <div
                    key={group.id}
                    className="flex items-center justify-between px-2 py-1.5 text-xs rounded hover:bg-[var(--surface-raised)] transition-colors"
                  >
                    <span className="flex items-center gap-1.5 truncate">
                      <span className={`w-3 h-3 rounded-full shrink-0 ${colorDef}`} />
                      <span className="truncate text-[var(--text-secondary)]">{group.name}</span>
                      <span className="text-[10px] text-[var(--text-muted)]">({group.nodeIds.length})</span>
                    </span>
                    <button
                      onClick={() => setEditingGroupId(group.id)}
                      className="p-0.5 hover:bg-[var(--border-highlight)] rounded"
                      title="Edit group"
                    >
      ...
      >
        ...
        <div className="p-3 border-b border-[var(--border-color)] bg-[var(--surface-raised)]">
          <div className="flex items-start justify-between">
            <div>
              <span className="inline-block px-2 py-0.5 rounded text-[11px] font-semibold border border-[var(--border-color)] text-[var(--text-secondary)] bg-[var(--surface)]">
                Multi Selection
              </span>
              <h3 className="mt-2 font-semibold text-sm text-[var(--text-primary)]">
                {selectedNodes.length} nodes selected
              </h3>
              <div className="mt-1 text-[11px] text-[var(--text-secondary)] break-all">
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={() => setIsCollapsed(true)}
                className="p-1 hover:bg-[var(--border-highlight)] rounded"
                title="Collapse panel"
              >
                <svg className="w-4 h-4 text-[var(--text-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 5l7 7-7 7M5 5l7 7-7 7" />
        ...
        <div className="p-3 border-b border-[var(--border-color)]">
          <h4 className="text-xs font-semibold text-[var(--text-secondary)] mb-2 tracking-wide">Annotate</h4>
          <BulkHighlightPicker
          ...
          <button
            ...
            className="mt-2 w-full px-2.5 py-1.5 text-xs font-medium text-emerald-400 bg-emerald-900/30 border border-emerald-800 rounded-md hover:bg-emerald-900/50 transition-colors"
          >
        ...
        <div className="p-3 border-b border-[var(--border-color)]">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-semibold text-[var(--text-secondary)] tracking-wide">{indicator.title}</span>
            <span className="text-xs text-[var(--text-secondary)]">{indicator.percentText}% {indicator.referenceLabel}</span>
          </div>
          <div className="mb-2 text-xs font-medium text-[var(--text-secondary)]">{indicator.formattedValue}</div>
        ...
        {parsedPlan?.hasActualStats && (
          <div className="p-3 border-b border-[var(--border-color)]">
            <h4 className="text-xs font-semibold text-[var(--text-secondary)] mb-2 tracking-wide">Actual Statistics (Selection)</h4>
        ...
        <div className="p-3 border-b border-[var(--border-color)]">
          <h4 className="text-xs font-semibold text-[var(--text-secondary)] mb-2 tracking-wide">
        ...
        {(aggregateSelection.sumMemoryUsed > 0 ||
          ...
          <div className="p-3 border-t border-[var(--border-color)]">
            <h4 className="text-xs font-semibold text-[var(--text-secondary)] mb-2 tracking-wide">Memory & I/O (Selection)</h4>
            <div className="grid grid-cols-2 gap-3">
  ...
  return (
    ...
    >
      ...
      />
      {/* Header */}
      <div className={`p-3 border-b border-[var(--border-color)] ${colors.bg}`}>
        <div className="flex items-start justify-between">
          <div>
        ...
        {(node.queryBlock || node.objectAlias) && (
          <div className="mt-2 flex flex-wrap gap-2">
            {node.queryBlock && (
              <span className="px-2 py-0.5 rounded text-xs font-medium bg-violet-900/30 text-violet-300">
                {node.queryBlock}
              </span>
            )}
            {node.objectAlias && (
              <span className="px-2 py-0.5 rounded text-xs font-medium bg-[var(--surface-raised)] text-[var(--text-secondary)]">
                {node.objectAlias}
              </span>
            )}
          </div>
        ...
        {(() => {
          ...
          return tip ? (
            <div className="mt-2 text-[11px] text-[var(--text-muted)] italic leading-snug">
              {tip}
            </div>
          ) : null;
        })()}
      ...
      {node.tempUsed !== undefined && node.tempUsed > 0 && (
        <div className="p-3 border-b border-[var(--border-color)] bg-yellow-950/30">
      ...
      {parsedPlan?.hasActualStats && (
        <div className="p-3 border-b border-[var(--border-color)]">
          <h4 className="text-xs font-semibold text-[var(--text-secondary)] mb-2 tracking-wide">Actual Statistics</h4>
          <div className="grid grid-cols-2 gap-3">
      ...
      {(node.accessPredicates || node.filterPredicates) && (
        <div className="p-3">
          <h4 className="text-xs font-semibold text-[var(--text-secondary)] mb-2 tracking-wide">Predicates</h4>

          {node.accessPredicates && (
            <div className="mb-3">
              <div className="flex items-center gap-2 mb-1">
                <span className="px-2 py-0.5 bg-green-900/30 text-green-400 text-xs rounded font-medium">
                  Access
                </span>
                <CopyButton text={node.accessPredicates} label="Copy access predicate" />
              </div>
              <code className="block text-xs bg-[var(--app-bg)] p-2 rounded border border-[var(--border-color)] text-[var(--text-primary)] font-mono whitespace-pre-wrap break-all">
          ...
          {node.filterPredicates && (
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="px-2 py-0.5 bg-amber-900/30 text-amber-400 text-xs rounded font-medium">
                  Filter
                </span>
      ...
      {(node.memoryUsed !== undefined ||
        ...
        <div className="p-3 border-t border-[var(--border-color)]">
          <h4 className="text-xs font-semibold text-[var(--text-secondary)] mb-2 tracking-wide">Memory & I/O</h4>
          <div className="grid grid-cols-2 gap-3">
...
function StatItem({ label, value, highlight }: { label: string; value?: string; highlight?: 'blue' | 'purple' | 'orange' }) {
  ...
  const highlightStyles = {
    ...
    orange: 'bg-orange-900/30 border border-orange-800',
  };
  ...
  return (
    <div className={`rounded-md p-2 border border-[var(--border-color)] ${highlight ? highlightStyles[highlight] : 'bg-[var(--app-bg)]'}`}>
      <div className="text-[11px] text-[var(--text-muted)] tracking-wide">{label}</div>
      <div className={`text-xs font-semibold ${highlight ? valueStyles[highlight] : 'text-[var(--text-primary)]'}`}>{value}</div>
    </div>
  );
}
...
function computeNodeDetailIndicator(
  ...
  return {
    ...
    color: clampedRatio === 0 ? 'bg-[var(--border-color)]' : getMetricColor(clampedRatio),
  };
}
