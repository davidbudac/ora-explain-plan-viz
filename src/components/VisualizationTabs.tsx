    ...
    <button
      ...
      className={`
        ...
        ${current === metric
          ...
          : 'text-[var(--text-secondary)] dark:text-[var(--text-secondary-dark)] hover:bg-[var(--surface-raised)] dark:hover:bg-[var(--surface-raised-dark)]'
        }
      `}
    >
      {label}
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-[var(--surface)] dark:bg-[var(--surface-dark)] border-b border-[var(--border-color)] dark:border-[var(--border-color-dark)] gap-2">
          ...
          {tabs.filter(tab => tab.id !== 'compare').map((tab) => (
            <button
              ...
              className={`
                ...
                ${
                  viewMode === tab.id
                    ? 'bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 border border-stone-900 dark:border-stone-100 shadow-sm'
                    : 'text-[var(--text-secondary)] dark:text-[var(--text-secondary-dark)] border border-[var(--border-color)] dark:border-[var(--border-color-dark)] hover:shadow-md shadow-sm'
                }
              `}
            >
              {tab.icon}
        ...
        {viewMode === 'hierarchical' && (
          <div className="flex items-center gap-2">
            ...
            <div className="flex bg-[var(--surface-raised)] dark:bg-[var(--surface-raised-dark)] rounded-full p-0.5 border border-[var(--border-color)] dark:border-[var(--border-color-dark)] shadow-sm">
              <IndicatorButton
                metric="cost"
                label="Cost"
                current={nodeIndicatorMetric}
      ...
      <div className="flex-1 min-h-0 h-full">
        ...
        {viewMode === 'text' && (
          <div className="h-full overflow-auto bg-[var(--surface-raised)] dark:bg-[var(--surface-raised-dark)] p-4">
