  ...
  return (
    ...
    >
      ...
      <div className="p-3 pt-4">
        {/* Operation ID badge */}
        <div className="absolute -top-2 -left-2 w-6 h-6 rounded-md bg-cyan-600 text-white text-xs font-bold flex items-center justify-center shadow">
          {node.id}
        </div>
        ...
        {(showHot || (hasSpill && options.showSpillBadge) || (options.showCardinalityBadge && cardSeverity !== 'good' && cardLabel)) && (
          <div className="flex flex-wrap gap-1 mb-1.5">
            {showHot && (
              <span className="px-1.5 py-0.5 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 text-[10px] rounded-md font-semibold flex items-center gap-0.5" title="Highest execution time in plan">
            ...
            {hasSpill && options.showSpillBadge && (
              <span className="px-1.5 py-0.5 bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300 text-[10px] rounded-md font-semibold" title="Spill to disk — temp space used">
                Spill
              </span>
            )}
            {options.showCardinalityBadge && cardSeverity !== 'good' && cardLabel && (
              <span
                className={`px-1.5 py-0.5 text-[10px] rounded-md font-semibold ${
                  cardSeverity === 'bad'
        ...
        {options.showQueryBlockBadge && node.queryBlock && (
          <div className="flex flex-wrap gap-1 mb-2">
            <span className={`px-1.5 py-0.5 text-xs rounded-md font-mono ${
              isMono
            ...
            {node.objectAlias && (
              <span className="px-1.5 py-0.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs rounded-md font-mono">
                {node.objectAlias}
              </span>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          {options.showRows && node.rows !== undefined && (
            <span className="px-1.5 py-0.5 bg-white/50 dark:bg-black/20 rounded-md text-gray-700 dark:text-gray-300">
