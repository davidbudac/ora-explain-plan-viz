...
function PlanNodeComponent({ data }: PlanNodeProps) {
  ...
  return (
    <div
      className={`
        relative w-[260px] rounded-xl ${borderClass} shadow-sm transition-all duration-200
      ...
      style={{ opacity }}
    >
      <Handle type="target" position={Position.Top} className="!bg-stone-400 dark:!bg-stone-500 !w-3 !h-3" />
      ...
      <div className="p-3 pt-4">
        {/* Operation ID badge */}
        <div className="absolute -top-2 -left-2 w-6 h-6 rounded-lg bg-stone-800 dark:bg-stone-200 text-white dark:text-stone-800 text-xs font-bold flex items-center justify-center shadow">
          {node.id}
        </div>
        ...
        {(showHot || (hasSpill && options.showSpillBadge) || (options.showCardinalityBadge && cardSeverity !== 'good' && cardLabel)) && (
          <div className="flex flex-wrap gap-1 mb-1.5">
            {showHot && (
              <span className="px-1.5 py-0.5 bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 text-[10px] rounded-full font-semibold flex items-center gap-0.5" title="Highest execution time in plan">
            ...
            {hasSpill && options.showSpillBadge && (
              <span className="px-1.5 py-0.5 bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300 text-[10px] rounded-full font-semibold" title="Spill to disk — temp space used">
                Spill
              </span>
            )}
        ...
        {options.showObjectName && node.objectName && (
          <div className="text-sm font-semibold font-mono text-stone-700 dark:text-stone-200 mb-2 truncate">
        ...
        {options.showQueryBlockBadge && node.queryBlock && (
          <div className="flex flex-wrap gap-1 mb-2">
            ...
            {node.objectAlias && (
              <span className="px-1.5 py-0.5 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs rounded-full font-mono">
                {node.objectAlias}
              </span>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          {options.showRows && node.rows !== undefined && (
            <span className="px-1.5 py-0.5 bg-white/50 dark:bg-black/20 rounded-full text-gray-700 dark:text-gray-300">
      ...
      <Handle type="source" position={Position.Bottom} className="!bg-stone-400 dark:!bg-stone-500 !w-3 !h-3" />
    </div>
  );
}
