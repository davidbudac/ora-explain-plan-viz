...
export function FilterPanel({ panelWidth, onResizeStart }: FilterPanelProps) {
  ...
  if (isCollapsed) {
    return (
      <div className="bg-[var(--surface)] border-r border-[var(--border-color)] flex flex-col items-center py-3">
        <button
        ...
        <span className="text-[11px] text-[var(--text-muted)] mt-2 writing-mode-vertical">Filters</span>
      </div>
    );
  }
  ...
  return (
    <div
      className="relative shrink-0 bg-[var(--surface)] border-r border-[var(--border-color)] overflow-y-auto"
      style={{ width: panelWidth }}
    >
      <button
        type="button"
        onPointerDown={onResizeStart}
        className="absolute right-0 top-0 z-10 h-full w-2 cursor-col-resize touch-none bg-transparent hover:bg-[var(--border-highlight)] transition-colors"
      ...
      {/* Search */}
      <div className="p-3 border-b border-[var(--border-color)]">
        <label className="block text-xs font-semibold text-[var(--color-primary)] opacity-80 mb-2 uppercase tracking-wide">
          Search
        </label>
        <input
          type="text"
          ...
          className="w-full px-2.5 py-1.5 text-xs border border-[var(--border-color)] rounded-md bg-[var(--app-bg)] text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-emerald-500/60"
        />
            ...
            {searchMatches.length > 0 && (
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="px-2 py-1 rounded border border-[var(--border-color)] bg-[var(--surface-raised)] text-[var(--text-secondary)] hover:bg-[var(--border-highlight)] transition-colors"
      ...
      {/* Predicate Types */}
      <div className="p-3 border-b border-[var(--border-color)]">
        <label className="block text-xs font-semibold text-[var(--color-primary)] opacity-80 mb-3 uppercase tracking-wide">
          Predicate Types
        </label>
                ...
                className={`
                  ...
                  ${
                    isActive
                      ...
                      : 'bg-[var(--surface-raised)] text-[var(--text-secondary)] border border-[var(--border-color)] hover:bg-[var(--border-highlight)]'
                  }
                `}
              >
                <span className="truncate">{label}</span>
                <span className="ml-2 px-2 py-0.5 bg-[var(--surface)] rounded text-[11px]">
                  {count}
                </span>
              </button>
            );
