/**
 * AI streaming via the hosted oraplanviz cloud backend (SaaS).
 *
 * The backend holds the model credentials server-side; the browser
 * authenticates with a per-account token. Wire contract (see
 * docs/plans/ai-plan-analysis.md, Phase 1.5 — service lives in the
 * separate `oraplanviz-cloud` repo):
 *
 *   POST {baseUrl}/v1/analyze   Authorization: Bearer <account token>
 *   Body: { system, prompt, model|null, maxTokens|null, kind: 'analyze' }
 *   → 200 text/event-stream:
 *       event delta {"text"} · event done {"stopReason","explanation"?}
 *       · event error {"message","status"?}
 *   → 401/402/429/500 JSON {"error","code"} before streaming
 */
...
/** Connection config for the hosted backend. */
...
function errorKindForStatus(status: number | null, message: string): AiError {
  ...
  if (status === 402) {
    return new AiError(
      'auth',
      `${message} — your oraplanviz cloud subscription is inactive or its usage limit is reached`,
      status,
    );
  }
...
/** Streams an AI analysis through the hosted oraplanviz cloud backend. */
export async function* streamHosted(
  config: HostedConfig,
  req: AiRequest,
  signal: AbortSignal,
): AsyncGenerator<AiStreamEvent> {
  const url = `${config.baseUrl.replace(/\/+$/, '')}/v1/analyze`;
  ...
  if (!response.ok) {
    const message =
      (await readErrorMessage(response)) ?? `Hosted AI request failed (HTTP ${response.status})`;
  ...
  try {
    for await (const message of parseSseStream(reader)) {
      ...
      if (message.event === 'error') {
        ...
        const errMessage = typeof payload.message === 'string' ? payload.message : 'Hosted AI stream error';
